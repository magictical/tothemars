...
export function RegistrationModal({ language, isOpen, onClose }: RegistrationModalProps) {
      ...
      <DialogContent className="bg-space-blue text-white border-white/10 sm:max-w-md">
        {!isSuccess ? (
          ...
          <div className="py-8 text-center space-y-4">
            ...
            <DialogTitle className="text-2xl font-bold">{t.success.title}</DialogTitle>
            <p className="text-gray-400">
              {t.success.message}
            </p>
            <Button onClick={handleClose} className="w-full mt-6 bg-white/10 hover:bg-white/20">
              {t.success.close}
            </Button>
          </div>
        )}
